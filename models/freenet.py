...
class FreeNet(nn.Module):
    ...
    def top_down(self, top, lateral):
        top2x = F.interpolate(top, (lateral.shape[2],lateral.shape[3]), mode='nearest')
...
def freenet(in_ch,num_classes,  **kwargs):
    """Constructs a Octave ResNet-26 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = FreeNet(in_ch,num_classes,block_channels=(96, 128, 192, 256),num_blocks=(1, 1, 1, 1), inner_dim=128,reduction_ratio=1, **kwargs)
    return model