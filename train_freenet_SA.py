...
def train(args, epoch, net, optimizer, trn_loader, criterion, categories):
    ...
    for idx, (X_data, y_target) in enumerate(trn_loader):
        ...
        #X_data=torch.cat((X_data,X_data), dim=0)
        y_target = Variable(y_target.float().long()).cuda(non_blocking=True)
        #y_target=torch.cat((y_target,y_target), dim=0)

        '''
        X_data_list= torch.chunk(X_data, 4, dim=2)        #切分
        y_target_list= torch.chunk(y_target, 4, dim=1)
        X_data_split=X_data_list[0]
        y_target_split=y_target_list[0]
        y_pred=torch.tensor(np.zeros((4,categories-1,X_data_split.shape[2],X_data_split.shape[3]))).cuda()
        gross_loss=0
        predicted=torch.tensor(np.zeros((4,X_data_split.shape[2],X_data_split.shape[3]))).cuda()
        for i in range(len(X_data_list)):
            y_pred[i,:,:,:] = net.forward(X_data_list[i])
            x=y_pred[i,:,:,:]
            _, predicted[i,:,:] = torch.max(x.reshape(1,x.shape[0],x.shape[1],x.shape[2]), 1)
            loss=criterion(x.reshape(1,x.shape[0],x.shape[1],x.shape[2]),y_target_list[i])
            gross_loss=gross_loss+loss
        '''

        #y_target1 = y_target.reshape(-class_num.py, class_num.py,y_target.shape[class_num.py]* y_target.shape[2])
        #y_target1=make_one_hot(y_target1,17)
        #y_target1 = y_target1.reshape(class_num.py, -class_num.py, y_pred.shape[2],y_pred.shape[3])
        #y_target1 = y_target1[:,class_num.py:,:,:].cuda()


        #loss1 = criterion2(y_pred, y_target1)



        y_pred = net.forward(X_data)
        #flops, params = profile(net, inputs=(X_data,))
        #print(flops)     #计算浮点数
        #print(params)    #模型参数量
        ...
        '''
        if (idx + class_num.py) % args.print_freq == 0:
            print('Epoch: [{}/{}][{}/{}], '
                  'Batch Loss {loss_meter.val:.4f}, '
                  'Accuracy {accuracy:.4f}.'.format(epoch + class_num.py, args.epochs, idx + class_num.py, len(trn_loader),
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='salina',
                        ...
                        help='dataset name')
    ## network setting
    parser.add_argument('--network', type=str, default='freenet',
                        choices=['FContNet','DeepLabV3Plus','SETR','SegNet','UNET','CBAM','octave','freenet'],
                        help='network name')
       ...
       # plt.pause(20)
        ...
        if args.input_mode == 'whole':
            ...
            with torch.no_grad():
                ...
                else:  pred = net(X_data.float().cuda())
                pred = pred.cpu().numpy()
                y_tes_pred = np.argmax(pred, 1)
        ...
        if h_ratio>1 or w_ratio>1:
            ...
            y_tes_gt=y_tes_data_1d[all_num]
            y_tes=y_tes_pred_1d[all_num]
            generate_png(y_tes_gt, y_tes,FLAG,y_tes_map.shape[0],y_tes_map.shape[1], all_num) #画图
        ...
        #y_cmap = y_tes_pred_1d
        #y_cmap+=1
        #y_cmap[list(set(all_num) - set(tes_num))]=0
        #y_cmap = y_cmap.reshape(r, c)
        #ground_truth = spectral.imshow(classes=y_cmap.astype(int), figsize=(9, 9))
        #plt.pause(20)
